...
geom_bar(aes(y = (..count..)/sum(..count..)), width=.5, fill = "steelblue") +
  ...
  coord_flip()+
  labs(title = "English level on arrival", x ="Proficiency", y = "Percent")
...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Employment Dashboard"),

    # Sidebar with selection options for the different crime types
    sidebarLayout(
        sidebarPanel(
            selectInput(inputId = "nationality",choices = unique(ASR_educ$nationality),
                        label = "Select Nationality",selected = "nationality")
        ),
...
server <- function(input, output) {
   ...
   output$map <- renderPlot({
    ...
    ggplot(aes(x=nationality, y=yearseduc))+
  geom_point(col="tomato2", size=3) +   # Draw points
  geom_segment(aes(x=nationality, 
                   xend=nationality, 
                   y=min(yearseduc), 
...
uix <- fluidPage(
  ...
  mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Training", plotOutput("training")),
  ...
  fluidRow(
    column(width = 5,
       actionButton("action", label = "Employment and Education"),
...
server_menu <- function(input, output) {
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Training", icon = icon("th"), tabName = "training"),
      menuItem("Demographics", icon = icon("th"), tabName = "demographics")
      )
    ),
  ...
  dashboardBody(
    ...
    plotOutput("map"),
    tabItems(
      tabItem("dashboard", "Widgets tab content"),
      tabItem("widgets", "Widgets tab content"),
      tabItem("Demographics", "Widgets tab content")
    )
  )
)
...
d <- reactive({
  ...
  output$map <- renderPlot({
   ggplot(data=subset(ASR_educ, !is.na(d)), aes(x = as.factor(d))) + 
geom_bar(aes(y = (..count..)/sum(..count..)), width=.5, fill = "steelblue") +
    geom_text(aes(y = ((..count..)/sum(..count..)), label = scales::percent((..count..)/sum(..count..))), stat = "count", hjust=-.1) +
  scale_y_continuous(labels = percent) +
  coord_flip()+
  labs(title = input$tabs, x = "Job type", y = "Percent")
   })
}
