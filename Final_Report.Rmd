---
title: "Project Report"
author: "R against the Machine"
date: "4/27/2019"
output:
  pdf_document: 
  html_document:
    df_print: paged
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
require(recipes)
require(tidyverse)
require(pdp)
require(utf8)
require("skimr")
require(foreign)
require(readr)
library(haven)
require(scales)
require(dplyr)
require(reshape2)
require(maps)
```

```{r}
 ASRraw <- read_dta("2016-ASR_Public_Use_File.dta") ##Loading main dataset
```
## PROBLEM STATEMENT AND BACKGROUND
Since 1975, the United States government has welcomed over 3 million refugees for resettlement from all over the world, allowing them to build new lives and contribute to the economies of all 50 states (UNHCR, 2018). In recent years, sharp cuts to refugee resettlement quotas have sparked debate over the program’s national security implications as well as the ability of refugees to integrate into their host communities. We found that this conversation is often driven by emotional and ideological claims rather than evidence.  In light of this, we ask: what does the most recent available data tell us about refugee integration outcomes in the United States?

We seek to answer this question by looking at the Annual Survey of Refugees 2016 (ASR), the most recent, nationally representative survey of refugees who were resettled in the US between 2011 and 2015. The survey was carried out by the Office of Refugee Resettlement at the U.S.Department of Health and Human Services (HHS) and offers a window into respondents' first five years in the US and their progress towards learning English, participating in the workforce, and establishing permanent residence. The dataset includes information on 1,500 households and more than 4,000 individuals, and is available as a STATA database to researchers from accredited universities.

We recognize that survey data can come with problems in terms of its reliability and external validity. However, for this particularly policy area, there is a huge gap in capturing refugee data with very few agencies collecting information (other than ASR, the only other data sources are the Census Bureau data: American Community Survey (ACS) and the New Immigrant Survey). 

Data science tools can help build the knowledge base regarding refugee integration and equip governments with tools to optimize their institutional procedures. This can have significant consequences for the long-term livelihood of both refugees and their host communities. For instance, scientists at Stanford University have developed a flexible data-driven algorithm that uses a combination of supervised machine learning and optimal matching to discover synergies between refugee characteristics and resettlement sites, leading to roughly 40 to 70% improvements, on average, in refugees’ employment outcomes relative to current assignment practices (Bansak et al., 2018).  

## METHODS
We answered our research question by relying on Exploratory Data Analysis (EDA) as our main approach. From the methodologies available to us, EDA was seen as a great fit with the nature of our project because it allowed us to open-mindedly explore and gain significant insight into the data. Moreover, we were drawn by the potential that data visualization, the backbone of the EDA methodology, could have to communicate our findings with the general public. In early stages, we considered using a more statistical approach, seeking to create a linear probability or machine learning to essentially predict the likelihood of a refugee household being successful at resettling. However, this approach was discarded because of issues related to data availability, the complexity of defining resettlement success in strictly quantitative measures, and in search of a more user-friendly approach that could maximize our project's reach beyond specialized audiences. 

Instead of creating a static report with data visualizations, we summarize the results of our EDA with the help of a dashboard that enables users to navigate the various dimensions of refugee integration, comparing and contrasting the outcomes attained with the help of charts. Our choice of a dashboard was driven by the significant popularity that they have gained (beyond their traditional niche in business analytics, and have become a powerful tool to communicate information on news outlets and think-tank, research settings. 

As a step prior to EDA, we did basic data cleaning and wrangling (cleaning, filtering appropriate variables) tasks to ensure the data could be rendered into visualizations.  We then coded an interface that allows users to navigate the different visualizations and explore the various dimensions of refugee integration, comparing and contrasting the outcomes attained in these areas. The dashboard is a dynamic tool that that provides concise explanations and guides the user/reader to understand the answers we have found to the research question.

## TOOLS

In this sense, we will rely on tidyverse package for general data analysis and modelling and ggplot2 for the creation of data visualizations. We plan to use the flexdashboard and Shiny packages to integrate a group of related data visualizations into a single environment that is friendlier to readers/users.

First, we used Github for version control. We used the following packages: recipes, tidyverse, pdp, utf8, skimr, foreign, Ereadr, haven for data manipulation, and scales and ggplot2 for our visualizations. Additionally, we used shiny, shinydashboard, and shinythemes for our dashboard.   

Problems: Github was often finicky and required manual interventions for debugging purposes. Additionally, we tried to think of ways to make our dashboard more interactive but the data itself did not much allow for this. However, the user will be able to click through different tabs, and the 'Schooling' subtab below 'Employment' has a drop-down menu with several variables to choose from. 

## Diya, add something about the map challenges 
Using R to make GIS visualizations is sometimes less efficient than using other softwares like Tableau. And the quality of the graphs are also not as good.

## Rafa, any challenges creating Shiny? Should we add anything that worked particularly well? Other than the tabs and adding paragraphs? 

## no idea how to improve any packages.. NOOOO idea. 

RESULTS
## RAFA - add your results here 
EMPLOYMENT AND SCHOOLING


## Benefits
BENEFITS

The policy debate in the United States is dominated by the question of how much of an economic burden refugees put on the tax system. For this reason, we created visualizations showing how many refugees in our sample have chosen to use government benefits in the past 12 months.  The visualizations show that for cash asssistance programs, few refugees receive them. For one, cash assistance programs have a higher income threshold. Additionally, the "I don't know" category is more pronounced than for cash asssistance programs, alluding to limitations of the data. In other words, it is possible that survey takers knew about assistance for SSI and SNAP, but not about cash assistance programs such as TANF. Further, specifically TANF and RCA have eligibility barriers; e.g. RCA is only available for the first five months, and TANF is limited to a cumulative lifetime of up to 5 years. Overall, however, less than 5% of survey takers utilize cash assistance program; the exception here is SSI, but even those are outweighted by those that do not receive SSI. Lastly, the outlier is SNAP; this is likely the case because income elibigilty thresholds are much lower and are available for households with children. 

Overall, 
```{r}
#### Steffi: I create a new dataframe with the variables in which I am interested (benefits) ##
ASR_ben <-
ASRraw %>%
select (qn30a, qn31a, qn32a, qn33a, qn34a)%>%
rename ("food"="qn30a") %>%  
rename ("tanf"="qn31a") %>% 
rename ("rca"="qn32a") %>% 
rename ("ssi"="qn33a") %>% 
rename ("ga"="qn34a")
 
### Steffi: Graphs ##
b1 <- ggplot(ASR_ben, aes(x = as.factor(food))) +
  labs(title = "SNAP") +
  geom_bar(position='dodge', width=.5, fill = "steelblue") +
  coord_flip() +
  scale_x_discrete(labels=c("1" = "No", "2" = "Yes",
                              "8" = "Don't know", "9" = "Refused")) +
  xlab("") +
  ylab("")

b2 <- ggplot(ASR_ben, aes(x = as.factor(tanf))) +
  labs(title = "TANF") +
  geom_bar(width=.5, fill = "steelblue") +
  coord_flip() +
  scale_x_discrete(labels=c("1" = "No", "2" = "Yes",
                              "8" = "Don't know", "9" = "Refused")) +
  xlab("") +
  ylab("")

b3 <- ggplot(ASR_ben, aes(x = as.factor(rca))) +
  labs(title = "RCA") +
  geom_bar(width=.5, fill = "steelblue") +
  coord_flip() +
    scale_x_discrete(labels=c("1" = "No", "2" = "Yes",
                              "8" = "Don't know", "9" = "Refused")) +
  xlab("") +
  ylab("")

b4 <- ggplot(ASR_ben, aes(x = as.factor(ssi))) +
  labs(title = "SSI") +
  geom_bar(width=.5, fill = "steelblue") +
  coord_flip() +
  scale_x_discrete(labels=c("1" = "No", "2" = "Yes",
                              "8" = "Don't know", "9" = "Refused")) + 
  xlab("") +
  ylab("")

b5 <- ggplot(ASR_ben, aes(x = as.factor(ga))) +
  labs(title = "GA") +
  geom_bar(width=.5, fill = "steelblue") +
  coord_flip() +
  scale_x_discrete(labels=c("1" = "No", "2" = "Yes",
                              "8" = "Don't know", "9" = "Refused")) +
  xlab("") +
  ylab("")

### creates multiplot ###
multiplot <- function(..., plotlist=NULL, file, cols=3, layout=NULL) {
  library(grid)

  # Make a list from the ... arguments and plotlist
  plots <- c(list(...), plotlist)

  numPlots = length(plots)

  # If layout is NULL, then use 'cols' to determine layout
  if (is.null(layout)) {
    # Make the panel
    # ncol: Number of columns of plots
    # nrow: Number of rows needed, calculated from # of cols
    layout <- matrix(seq(1, cols * ceiling(numPlots/cols)),
                    ncol = cols, nrow = ceiling(numPlots/cols))
  }

 if (numPlots==1) {
    print(plots[[1]])

  } else {
    # Set up the page
    grid.newpage()
    pushViewport(viewport(layout = grid.layout(nrow(layout), ncol(layout))))

    # Make each plot, in the correct location
    for (i in 1:numPlots) {
      # Get the i,j matrix positions of the regions that contain this subplot
      matchidx <- as.data.frame(which(layout == i, arr.ind = TRUE))

      print(plots[[i]], vp = viewport(layout.pos.row = matchidx$row,
                                      layout.pos.col = matchidx$col))
    }
  }
}
```

```{r}
multiplot(b1, b2, b3, b4, b5, cols=2)
```
## Demograhics DIYA - add your results and visualizations here 
DEMOGRAPHICS
We reviewed the demographic composition of the refugee survey data in order to get more understanding about the refugees. This analysis looks into the overall situation of refugees in the United States and sets stage for the analysis of employment and benefits.By looking at the data and visualizations below,you should be able to understand the general distribution of the age , gender, country of birth as well as the state of resettlement distribution of the refugees. 

###selecting subset, renaming variables and summarize the data###
library(readstata13)
setwd("~/Downloads")



```{r}
df <-  read_dta("2016-ASR_Public_Use_File.dta")
data <- data.frame("age"=df$qn1d,"gender"=df$qn1f,"country of birth"=df$qn1g,"State originally resettle"=df$qn1k,"country of citizenship"=df$qn1h,"ethnic origin"=df$qn1i)

name <- c("age","gender","C_O_B","S_O_R","C_O_C","ethic_origin")
names(data)<-name
head(data)
table(data %>% select(C_O_B)) 
table(data %>% select(S_O_R))
table(data %>% select(gender)) 
table(data %>% select(age))
#age subset
data <- data %>% mutate(age_group =
                case_when(
                  age <= 15 ~ "0-15",
                  age <= 30 ~ "16-30",
                  age <= 45 ~ "31-45",
                  age <= 60 ~ "46-60",
                  age <= 75 ~ "61-75",) 
                )
#drop unreasonable data
data_filt <- data[data$age<=75,]

#country of birth subset

world_map = map_data("world")
cobnumber <- data %>% group_by(C_O_B) %>% 
summarize(count = n())
colnames(cobnumber)[colnames(cobnumber)=="C_O_B"] <- "region"
C_O_B.map <- right_join(cobnumber, world_map, by = "region")

#state of resettlement subset

regionnumber <- data %>% group_by("S_O_R") %>% summarize(count = n())
colnames(regionnumber)[colnames(regionnumber)=="S_O_R"] <- "area"
regionnumber$area[regionnumber$area==1] <- "Northeast"
regionnumber$area[regionnumber$area==2] <- "South"
regionnumber$area[regionnumber$area==3] <- "North Central"
regionnumber$area[regionnumber$area==4] <- "West"

###graphs###
#1
ggplot(data_filt, aes(x = age,fill=age_group)) +
  geom_histogram(aes(y=..density..))+
  facet_wrap(.~gender,scales="free")

#We can see from this graph that for both genders, the age distribution is generally equal, which means that there is not significant difference between male and female refugees.

#2
ggplot(data_filt,aes(x=age,y=count,fill=age_group))+geom_histogram(aes(y=..count..))

#We can see from this graph that age group 16-30 and 31-45 take the most count in refugee age distribution.

#3
ggplot(data = C_O_B.map, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = C_O_B.map$count))
  
#The largest group of refugees were born in Iraq, which counts 1567. following by 611 for Bhutan and 513 for Burma.From the map, we can see that the refugess are mainly from the Middle East, Southeat Asia, and Africa. 
                            
#4
ggplot2_states <- map_data("state")
ggplot2_states$region <- str_to_title(ggplot2_states$region)
region_data <- data.frame(region=state.name,area = state.region)
ggplot2_statesdata <- inner_join(ggplot2_states,region_data,"region")
ggplot2_statesdata <- inner_join(ggplot2_statesdata,regionnumber,"area")

g1 <- ggplot(data = ggplot2_statesdata,
             mapping = aes(x = long, y = lat, group = group,fill=count))+
  scale_fill_gradient(low = "#330000", high = "#FFFFFF") +
  geom_polygon(color="gray90",size=0.1)
g1

#The count for resettlement in each state is 744, 1402, 1382, 1155 respectively. We can see that the South and the North Central have very close resttlement counts, while the Northeast has the least count comparing to other three regions.



